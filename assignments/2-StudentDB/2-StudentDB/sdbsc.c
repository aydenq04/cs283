...
int get_student(int fd, int id, student_t *s){
	...
	//Read through the file until student is found
    ...
    return SRCH_NOT_FOUND; // Student was not found
}
...
int add_student(int fd, int id, char *fname, char *lname, int gpa){
    ...
    new_student.lname[sizeof(new_student.fname) - 1] = '\0';
    new_student.gpa = gpa;
...
int del_student(int fd, int id) {
    if (fd < 0) {
    	printf(M_ERR_DB_READ);
    	return ERR_DB_FILE; // Invalid file name
    }
    ...
    if (lseek(fd, 0, SEEK_SET) == -1) {
    	printf(M_ERR_DB_READ);
    	return ERR_DB_FILE; // Error while seeking
    }

    while (read(fd, &temp, sizeof(student_t)) == sizeof(student_t)) {
    	if (temp.id == id) {
    		offset = lseek(fd, 0, SEEK_CUR) - sizeof(student_t);
    		...
    		if (write(fd, &EMPTY_STUDENT_RECORD, sizeof(student_t)) != sizeof(student_t)) {
    			printf(M_ERR_DB_WRITE);
    			return ERR_DB_FILE; // Error while writing
    		}
