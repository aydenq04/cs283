...
void parse_cmd(char *cmd_line, cmd_buff_t *cmd) {
...
int parse_line(char* cmd_line, command_list_t *clist) {
...
int exec_client_requests(int cli_socket) {
    ...
    while(1) {
		...
		rc = parse_line(io_buff, &cmd_list);
		if (rc != OK) {
			send_message_string(cli_socket, "Error parsing command\n");
			continue;
		}

		if (cmd_list.num > 0 && strcmp(cmd_list.commands[0].argv[0], "exit") == 0) {
			free(io_buff);
			return OK;
		}

		if (cmd_list.num > 0 && strcmp(cmd_list.commands[0].argv[0], "stop-server") == 0) {
			free(io_buff);
			return OK_EXIT;
		}
...
int send_message_string(int cli_socket, char *buff){
    ...
    ssize_t sent_bytes;
    sent_bytes = send(cli_socket, buff, strlen(buff), 0);
    if (sent_bytes == -1) {
    	perror("send");
    	return ERR_RDSH_COMMUNICATION;
    }
